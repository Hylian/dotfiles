...
base_tail=$(cat <<EOF
  ...
  <!--
    Substitutions from Infinality guide: https://gist.github.com/cryzed/e002e7057435f02cc7894b9e748c5671
    Install AUR package: fonts-meta-extended-lt
  -->
  <alias>
...
echo "$base_head" > $HOME/.config/fontconfig/fonts.conf

case $1 in
  "rlcd")
    echo "Writing RLCD fontconfig"
    echo "$rlcd_config" >> $HOME/.config/fontconfig/fonts.conf
    ;;
  "oled")
    echo "Writing OLED fontconfig"
    echo "$oled_config" >> $HOME/.config/fontconfig/fonts.conf
    ;;
  "lcd")
    echo "Writing LCD fontconfig"
    echo "$lcd_config" >> $HOME/.config/fontconfig/fonts.conf
    ;;
  *)
    echo "Writing default (LCD) fontconfig"
    echo "$lcd_config" >> $HOME/.config/fontconfig/fonts.conf
    ;;
esac
echo "$base_tail" >> $HOME/.config/fontconfig/fonts.conf
