...
* **Universal Clipboard & Headless Sync:** Neovim yanks and deletes (`y`, `yy`, `d`, `dw`, `cw`, `x`, etc.) copy to local display servers asynchronously via `vim.system` ONLY when display envvars (`WAYLAND_DISPLAY`, `DISPLAY`) are present (preventing `wl-copy`/`xclip` from hanging in headless/SSH sessions and eliminating UI loop blocking on IPC/forks), persist to `~/.cache/clipboard`, AND broadcast ANSI OSC 52 sequences to `/dev/tty` for terminal pasteboard sync (Ghostty on macOS). Handled directly via `vim.g.clipboard` register overrides without redundant `TextYankPost` hooks.
