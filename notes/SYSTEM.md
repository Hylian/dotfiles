...
* **Status Bar Repaint Kick:** zjstatus stores a command result without scheduling a render (`Event::RunCommandResult` never sets `should_render`), so a new branch waits for the 1s render timer — median 784ms after a pane switch. The widget therefore pipes `zjstatus::pipe::zjs_render_kick::-` (a pipe name no format references, so it only forces a render) whenever its value or directory changed, bringing repaint to a median of 319ms. `zellij pipe` is a *streaming* client: it must be given `</dev/null` and a plain-sh watchdog (macOS has no `timeout(1)`) or it blocks forever against a dead or wedged server, holding a socket each time — this exhausted file descriptors and crashed a server once. See [2026-07-24-repaint-git-widget-on-change.md](2026-07-24-repaint-git-widget-on-change.md).
