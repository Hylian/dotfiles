...
* **Status Bar & Instant Refresh:** `zjstatus` WASM plugin configured with 1s interval polling. Zsh `chpwd` hooks (`zellij_tab_name_update`, `_zellij_refresh_git_branch`) fire disowned background pipe triggers (`zjstatus::rerun::git_branch`) to eliminate latency on directory switches (`cd`, `^j`, `^k`), while `Event::PaneUpdate` natively invalidates `git_branch` on pane focus changes.
