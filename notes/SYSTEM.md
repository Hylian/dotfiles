...
* **Trackpad & Mouse Scroll Multiplier:** `mouse-scroll-multiplier = precision:4,discrete:3` configured on macOS (`darwin`). Pairs with Zellij's 1-line scroll event handling to provide high-frequency single-line step scrolling for smooth trackpad animation while preserving full finger sensitivity.
