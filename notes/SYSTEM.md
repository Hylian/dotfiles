...
* **Build Provenance (macOS client):** Zellij is built from the personal fork `Hylian/zellij`, branch `hylian/latency`, and installed to `~/.local/bin/zellij` (which precedes `/opt/homebrew/bin` on PATH); the brew package has been removed on this host. Rebuild with `cargo xtask install ~/.local/bin/zellij`, which needs `protobuf` (brew) and `mandown` (`cargo install --locked mandown`). Beware `brew uninstall` here: it autoremoves orphaned formulae, and removing zellij also swept `python@3.14`, `python@3.13`, `certifi`, `pydantic`, `cffi` and `pycparser` (all reinstalled since). Other hosts may still run stock zellij; every config below is written to work either way.
* **Fork Patch — Plugin Autorender:** `apply_event_to_plugin` forces `should_render` for `Event::RunCommandResult` the same way upstream already does for `PermissionRequestResult`, so a plugin's command output paints as soon as it lands instead of waiting for the next unrelated render. A second commit sets `ZELLIJ_PLUGIN_AUTORENDER=1` in the environment of plugin-spawned commands so widgets can detect the capability. Together these take pane-switch repaint from a median of 784ms (stock) to **114ms**, with the shell-side repaint kick switching itself off. See [2026-07-24-zellij-fork-plugin-autorender.md](2026-07-24-zellij-fork-plugin-autorender.md).
...
* **Status Bar Repaint Kick (stock-zellij fallback):** Stock zjstatus stores a command result without scheduling a render (`Event::RunCommandResult` never sets `should_render`), so a new branch waits for the 1s render timer — median 784ms after a pane switch. When `ZELLIJ_PLUGIN_AUTORENDER` is **absent** the widget pipes `zjstatus::pipe::zjs_render_kick::-` (a pipe name no format references, so it only forces a render) whenever its value or directory changed, giving a median of 319ms; on the patched build the marker is present and the kick is skipped entirely. `zellij pipe` is a *streaming* client: it needs `</dev/null` and a plain-sh watchdog (macOS has no `timeout(1)`) or it blocks forever against a dead or wedged server, holding a socket each time — this exhausted file descriptors and crashed a server once. The watchdog subshell must also redirect **stdout**, since zellij reads the command's output to EOF and a grandchild holding that pipe stalls the result for the full watchdog window. See [2026-07-24-repaint-git-widget-on-change.md](2026-07-24-repaint-git-widget-on-change.md).
