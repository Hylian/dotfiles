...
* **Status Bar & Event-Driven Refresh:** `zjstatus` `command_git_branch_interval` is set to `"0"` (timer polling disabled). Zsh registers `_zellij_refresh_git_branch` in `precmd` and `chpwd` to update `git_branch` once per prompt/directory switch, while `_zellij_osc7_cwd` (`\e]7;file://...`) syncs pane CWDs to Zellij server memory.
