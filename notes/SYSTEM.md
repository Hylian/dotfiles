...
* **Status Bar & Event-Driven Refresh:** `zjstatus` `command_git_branch_interval` is set to `"60"` (periodic polling reduced by 98%). Zsh registers `_zellij_refresh_git_branch` in `precmd` and `chpwd` to trigger instant disowned pipe reruns (`zjstatus::rerun::git_branch`) once per Zsh prompt/directory switch, while `_zellij_osc7_cwd` (`\e]7;file://...`) syncs pane CWDs to Zellij server memory.
* **Decoupled Shell IPC & Focus Reporting:** `\e[?1004h` enables terminal focus reporting inside Zellij. `zle-focus-in` emits lightweight ANSI OSC 7 (`\e]7;file://...`) immediately on every focus event and dispatches disowned `zjstatus::rerun::git_branch` rate-limited to at most once per 2 seconds, preventing process flooding during rapid pane switching (`Alt+h/j/k/l`). `zle-focus-out` absorbs `\e[O` escapes to prevent pink `[!]` bell alert flashes.
