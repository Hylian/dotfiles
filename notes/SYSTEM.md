...
* **Status Bar & Event-Driven Refresh:** `zjstatus` `command_git_branch_interval` is set to `"1"`. Zsh registers `_zellij_refresh_git_branch` in `precmd` and `chpwd` to trigger instant disowned pipe reruns (`zjstatus::rerun::git_branch`) once per Zsh prompt/directory switch, while `_zellij_osc7_cwd` (`\e]7;file://...`) syncs pane CWDs to Zellij server memory.
