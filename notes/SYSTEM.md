...
* **Shell Framework:** `zsh` + `antigen` with cached initialization in `$XDG_CACHE_HOME/zsh`. Automatically runs `antigen cache-gen` immediately after `antigen apply` whenever `.zshrc` is updated, regenerating `~/.antigen/init.zsh` to prevent permanent 1.5s `antigen apply` loops (`~20ms` warm `.zshrc` boot). Sets `ZSH_DISABLE_COMPFIX="true"` to disable Oh-My-Zsh `compaudit` directory permission audits during completion initialization. FZF directory jumping (`^g` and `vim-fzf`) uses `${FD_COMMAND:-fd}` to support Ubuntu/Debian systems where `fd` is `fdfind`.
